...
declare interface $ { // 更新 2022/06/29
  ...
  sum<T>(item: T[], callBack: (a: T , b: T, index?: number, arr?: T[]) => T): T
  sum<T, I>(item: T[], callBack: (a: I , b: T, index?: number, arr?: T[]) => I, initialVal: I): I
  ...
  createPromiseAll<T extends readonly unknown[]>(paramaters: T): Promise<{ [K in keyof T]: Awaited<T[K]> }>
...
$.sum = (...args: any[]) => { const [item, callback, initialVal] = args as [any[], any, any]; return initialVal ? item.reduce((a, b, index, arr) => callback.call(callback, a, b, index, arr), initialVal) : item.reduce((a, b, index, arr) => callback.call(callback, a, b, index, arr)) }
