    ...
    .body
      .list(v-if="ptLocation.length > 0")
        .list-item-row(v-for="(item, index) in ptLocation",:key="index")
          ...
          .list-item-col {{ convertTime(item.PDOnDate) }}
  ...
  setup(){
    ...
    const method: {
      ...
      getProduct: async pages => {
        ...
        const res = await Fetch.get<{ data: typeof pageState.ptLocation.value }>("/product/get?o=1", { token: token.value })
          
        if (res.status == 200) {

          if (res.data.data.length == 0) {
            pageState.stateText.value = "目前暫無上架商品";
            return;
          }
        ...
        pageState.stateText.value = "連線異常";
      }
    }

    onMounted(async () => {
      await method.getProduct(1);
    })
