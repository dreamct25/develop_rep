...
export interface Self extends HTMLElement {
  ...
  scrollToTop(scrollSetting: { scrollTop?: number, scrollLeft?: number, duration: number }): void
...
const $: $ = (target) => {
  ...
  self.scrollToTop = (scrollSetting) => { // 更新方法 2021/10/26
    ...
    (animateScroll = () => {
      ...
      self[scrollSetting?.scrollTop ? 'scrollTop' : 'scrollLeft'] = Number(animationSettings({
        currentTime: currentTimeStamp,
        startVal: startPos,
        changeVal: changePos,
        animateDuration: (scrollSetting as Record<string, any>)[keyII]
      }))
      currentTimeStamp < (scrollSetting as Record<string, any>)[keyII] ? requestAnimationFrame(animateScroll) : self[scrollSetting?.scrollTop ? 'scrollTop' : 'scrollLeft'] = (scrollSetting as Record<string, any>)[keyI]
    })()
  }
...
$.currencyTranser = (formatNumber, currencyType, withCurrencyStyle) => { // 更新方法 2022/06/24
  if ($.typeOf(formatNumber, 'Number')) {
    const currencyOptionalObj = !withCurrencyStyle ? undefined : { style: 'currency', currency: currencyType } as Intl.NumberFormatOptions
