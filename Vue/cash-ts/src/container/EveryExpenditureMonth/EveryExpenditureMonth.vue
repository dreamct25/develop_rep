<template lang="pug">
.page-outer(:class="{'page-outer-active':chooseCost}")
  span.cost-year-title {{ currentYear }} 年每月支出
  .row.justify-content-center
      .col-md-10
          .cost-year-choose-outer-frame
              .cost-year-choose-outer(:class="{'cost-year-choose-outer-active':chooseCost}")
                  .chart-layout
                    .cost-year-choose
                      |查詢年份
...
import { stateType,methodType,everyExpenditureMonthDataType } from '.'
...
export default defineComponent({
    ...
    setup(){
      ...
      const state:stateType = {
          ...
          everyExpenditureMonthData: ref([]),
          isLoading: ref(false)
      }

      const method:methodType = {
          goBack: () => {
            state.chooseCost.value = false
            setTimeout(() => router.push("/main"), 1000);
          },
          getData: async year => {
            state.isLoading.value = true

            const { status,data: result,statusText } = await Fetch.get<everyExpenditureMonthDataType[]>(`/get_every_month/expenditure/${year}`,{
              token: auth.value.token
            })

            if(status >= 200 && status <= 399){
                state.everyExpenditureMonthData.value = result
            } else if(status === 403) {
      ...
      watch(state.everyExpenditureMonthData,cur => {
      ...
      onMounted(async () => {
        ...
        setTimeout(() => state.chooseCost.value = true, 1900);
      })
