        ...
        <table class="table">
          ...
          <tbody>
            <tr v-for="(items, index) in ptLocation" :key="index">
              <td v-if="items.PDOffDate != null">{{ items.PDName }}</td>
              <td v-if="items.PDOnDate != null">
                $ {{ items.PDOriginPrice }} NT
              </td>
              <td v-if="items.PDOffDate != null">
                $ {{ items.PDOnSalePrice }} NT
              </td>
              <td v-if="items.PDOffDate != null">
                {{ items.PDPice }}
              </td>
              <td v-if="items.PDOffDate != null">{{ items.PDPice }} 個</td>
              <td v-if="items.PDOffDate != null">
        ...
        .then((res) => {
          if (res.status == 200) {
            res.data.forEach((key) =>
              Boolean(key.PDState) == false ? this.ptLocation.push(key) : null
            );
            this.ptLocation.length == 0
              ? (this.stateText = "目前暫無下架商品")
              : null;
          }
        })
