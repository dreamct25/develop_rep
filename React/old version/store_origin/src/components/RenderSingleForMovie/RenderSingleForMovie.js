...
import importItem from '../RenderSingleForMovie'

const { actions,styles } = importItem
const { Show } = styles

class RenderSingleForMovie extends Component{
    constructor(props){
        super(props)
            ...
            {this.props.singleData.map(item => (
                <div className="single-content" key={item.trackId} onMouseEnter={this.videoBarToggleChange.bind(this,true)} onMouseLeave={this.videoBarToggleChange.bind(this,false)}>
                    <div className={this.props.isPlayed === false || (this.props.isPlayed === true && this.props.videoBarToggle === true) ? "video-title-outer" : "video-title-outer video-title-outer-toggle"}>
                        <span className="video-title">{item.artistName}-{item.trackName} ( Trailer )</span>
                    </div>
    ...
    setVideo(){
        ...
        setTimeout(()=>{
            if(String(this.props.rangeMaxVal) === "NaN"){
...
RenderSingleForMovie.defaultProps = {
    voiceVal:50
}

const reactReduxStateToProps = state => ({
    singleData:state.getIn(['movieItem','singleData']),
    isPlayed:state.getIn(['renderSingleForMovie','isPlayed']),
    rangeVal:state.getIn(['renderSingleForMovie','rangeVal']),
    rangeMaxVal:state.getIn(['renderSingleForMovie','rangeMaxVal']),
    timer:state.getIn(['renderSingleForMovie','timer']),
    durationTime:state.getIn(['renderSingleForMovie','durationTime']),
    voiceVal:state.getIn(['renderSingleForMovie','voiceVal']),
    textShowToggle:state.getIn(['renderSingleForMovie','textShowToggle']),
    textShowContent:state.getIn(['renderSingleForMovie','textShowContent']),
    videoBarToggle:state.getIn(['renderSingleForMovie','videoBarToggle']),
    voiceValToggle:state.getIn(['renderSingleForMovie','voiceValToggle'])
})
...
export default connect(reactReduxStateToProps,reactReduxDispatchToProps)(RenderSingleForMovie)