...
const $: $ = ((el) => {
  ...
  $.formatDateTime = format => { // 更新方法 2021/12/01
    ...
    const dateStr: string = new Date($.typeOf(format.formatDate, 'Number') ? format.formatDate as number : +new Date(format.formatDate as string) + localCountryTime).toJSON()
    ...
    if (format.formatType.match('tt')) {
      ...
      return {
        ...
        getWeekName: format.customWeekItem![new Date(+new Date(format.formatDate as string)).getDay()]
      }
    } else {
