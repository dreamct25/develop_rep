...
// import { HiNetHichannel } from 'hinet-hichannel-taiwan-radio'

const route: Router = Router()

route.get('/',(req, res) => {
    res.status(200).json({ a: 10 })
})
...
route.get('/get_channel', (req: Request, res: Response<{
    ...
    //         })
    //     }).catch((err:any) => res.json({ message: err, data:{ channels:[],rankItem:[] } }))
    // }).catch((err:any) => res.json({ message:err, data:{ channels:[],rankItem:[] } }))
})
...
route.post('/get_single_channel', (req: Request, res: Response) => {
    ...
    //         hichannel.getChannelProgramInfo().then((info: any) => {
