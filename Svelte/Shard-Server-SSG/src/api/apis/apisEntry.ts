...
import trachBin from 'trash'
import fs from 'fs'
import path from 'path'
import os from 'os'
import { path as ffmpegPath } from '@ffmpeg-installer/ffmpeg';
import fmg from 'fluent-ffmpeg'
...
route.get('/check_setting_exist',async (req,res) => {
    const [settingPathBase,] = __dirname.split('src')
    const settingPath = path.join(settingPathBase,`usr/setting`)
    ...
    try {
        ...
        const context: SettingConfig = JSON.parse(
            decodeURIComponent(
        ...
        if(!fs.existsSync(context.upload_url)) {
            await fs.promises.mkdir(context.upload_url,{ recursive:true })
        }
...
route.post('/create_setting',async (req,res) => {
    ...
    if(req.body.u){
        ...
        try {
            const [settingPathBase,] = __dirname.split('src')
            const settingPath = path.join(settingPathBase,`usr/setting`)
            const settingDirPath = path.join(settingPathBase,'usr')
...
route.get('/get_remote_ip',(req,res) => {
    ...
    res.status(200).json({
        currentIp:`http://${addressIp}:9621`,
        isLocal: req.hostname === 'localhost'
    })
})

// upload file api
route.post('/upload_file',uploadFile.array('files'),(req,res) => res.json({ message:'trans.uploadStatus.uploaded' }))
