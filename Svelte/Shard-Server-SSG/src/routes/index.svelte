    ...
    <div class="main">
        {#if !rwdStatus}
            <div class="file-list-header">
                ...
                >
                    ...
                    <Tooltip pos={{ x: currentLanguage === 'en' ? -68 : -33,y:-67 }} width={currentLanguage === 'en' ? 150 : 80}>{$t('trans.prevFolder')}</Tooltip>
                </div>
                ...
                <div class="upload-progress-bar">
                    <div class={!['trans.uploadStatus.uploading','trans.uploadStatus.ready'].includes(progressTxt) ? 'upload-progress-bar-inner active' : 'upload-progress-bar-inner'} style="width:{count === -1 ? 0 : count}%;"></div>
                    <div class="progress-text">{['trans.uploadStatus.ready'].includes(progressTxt) ? $t(progressTxt) : `${$t(progressTxt)} `}{count !== -1 ? `${Math.floor(count)} %` : ''}</div>
                </div>
                <div class="fn-group">
                    <!-- svelte-ignore a11y-click-events-have-key-events -->
                    <div class={listShortIcon ? "fn active" : "fn"} on:click={() => listShortIcon = true}>
                        <FontAwesomeIcon icon={faGripHorizontal} />
                        <Tooltip pos={{ x:0,y:-40 }} width={70}>{$t('trans.shortList')}</Tooltip>
                    </div>
                    <!-- svelte-ignore a11y-click-events-have-key-events -->
                    <div class={!listShortIcon ? "fn active" : "fn"} on:click={() => listShortIcon = false}>
                        <FontAwesomeIcon icon={faList} />
                        <Tooltip pos={{ x:0,y:-40 }} width={70}>{$t('trans.detailList')}</Tooltip>
                    </div>
                    <!-- svelte-ignore a11y-click-events-have-key-events -->
                    <div class="fn" on:click={shortDate}>
                        <FontAwesomeIcon icon={listDataDateSort ? faArrowUpShortWide : faArrowDownWideShort} />
                        <Tooltip pos={{ x:0,y:-40 }} width={115}>{$t(listDataDateSort ? "trans.sortLateDate" : "trans.sortLatestDate")}</Tooltip>
                    </div>
                    <!-- svelte-ignore a11y-click-events-have-key-events -->
                    {#if currentIp}
                        <div class="fn">
        ...
        <div class="file-list-outer">
            {#if !listShortIcon}
                <div class="file-list">
                    ...
                    {:else}
                        {#if !rwdStatus}
                            <div class="no-data">{$t('trans.noAnyFile')}</div>
                        {:else}
                            <div 
                                class="no-data"
                                on:touchstart={(e) => touchsEventObj = e }
                                    ...
                                    <div class="file-list-item">
                                        {#if new RegExp('^.*\.(jpg|JPG|gif|GIF|jpeg|JPEG|png|PNG|TIFF|tiff|TIF|tif)$').test(items.fileType)}
                                            <div class="img-outer-frame">
                                                <div class={`loading ${items.fileLoadStatus ? '' : 'active'}`}>{$t('trans.loadingPhoto')}</div>
                                                <div class="img-outer">
                                                    <div class="img-frame">
                                                        <img
                                                            on:load={(event) => whenLoadImg(items.fileName,event)} 
                                                            src={`/preview?f=${btoa(encodeURIComponent(items.fileUrl))}`} 
                                                            alt="" 
                                                        />
                                                    </div>
                                                </div> 
            ...
            <div class="option-menu-list">
                {#each optionMenuItem as menuItem}
                    {#if menuItem.disabled}
                        <!-- svelte-ignore a11y-click-events-have-key-events -->
                        <div on:click={menuItem.useMethod} >{$t(menuItem.label)}</div>
                    {/if}
                {/each}
            </div>
        </div>
        <div class={toggleModal ? "modal-outer active" : "modal-outer"}>
            <div class="modal">
                <div class="modal-header">{
                    $t(currentUseMethod === 'create' ? "trans.modal.header.create" : "trans.modal.header.rename",{ default: haveExtendsName ? $t('trans.modal.header.file') : $t('trans.modal.header.folder') })}</div>
                <div class="modal-body">
                    <input type="text" bind:value={newDirName} />
                </div>
                <!-- svelte-ignore a11y-click-events-have-key-events -->
                <div class="modal-footer">
                    <div on:click={toggleModalStatus.bind(undefined,false,'confirm')}>{$t('trans.modal.footer.confirm')}</div>
                    <div on:click={toggleModalStatus.bind(undefined,false,'cancel')}>{$t('trans.modal.footer.cancel')}</div>
                </div>
            </div>
        </div>
        <div class={toggleDeleteModal ? "delete-modal-outer active" : "delete-modal-outer"}>
            <div class="modal">
                <div class="modal-header">{$t('trans.deleteModal.header')}</div>
                <div class="modal-body">
                    <span>{$t('trans.deleteModal.body',{ default:newDirName })}</span>
                </div>
                <!-- svelte-ignore a11y-click-events-have-key-events -->
                <div class="modal-footer">
                    <div on:click={toggleDeleteModalStatus.bind(undefined,false,'confirm')}>{$t('trans.deleteModal.footer.confirm')}</div>
                    <div on:click={toggleDeleteModalStatus.bind(undefined,false,'cancel')}>{$t('trans.deleteModal.footer.cancel')}</div>
                </div>
            </div>
        </div>
        ...
        <div class={`download-progress-bar-outer ${toggleDownloadModal ? 'active' : ''}`}>
            ...
            <div class="download-title">
                ...
                <div>
                    <span>{$t('trans.downloadStatusGroup.downloadFileName')}：</span>
                    <span class={language.get()}>{fullFileName || 'no file'}</span>
                </div>
                <div>
            ...
            <div class="progress-outer">
                {downloadStatusItem[downloadStatusCode] ? $t(downloadStatusItem[downloadStatusCode]) : `${downloadPercentCount.toFixed(2)} %`}
...
>
    <div class="modal-outer">
        <div class="modal-header">{$t((!getLoccalSettingStatus && getLoccalSettingStatus !== undefined) ? 'inputModal.header' : 'inputModal.headerNotFirstTime')}</div>
        <div class="modal-body">
            <input type="text" class={getLoccalSettingStatus ? 'without-bottom' : ''} bind:value={createUploadPath}>
            {#if !getLoccalSettingStatus}
                <div>{$t('trans.inputModal.body')}</div>
            {/if}
        </div>
        <div class="modal-footer">
            <!-- svelte-ignore a11y-click-events-have-key-events -->
            <div class="setting-btn" on:click={settingEvent}>{$t('trans.inputModal.footerBtn')}</div>
        </div>
    </div>
</div>
<div class="footer">
    <h6>{$t('trans.copyRight')}</h6>
</div>
<script lang="ts">
    ...
    import { t, locale as language } from '../i18n'
    import { 
        faList,
        faGripHorizontal,
        faFolderClosed,
    ...
    let progressTxt:string = 'trans.uploadStatus.ready'
    let count:number = -1
    ...
    const downloadStatusItem: Record<number,string> = {
        [downloadStatusCode]: '',
        0: 'trans.downloadStatusGroup.prepareDownload',
        4: 'trans.downloadStatusGroup.convertFile', 
        5: 'trans.downloadStatusGroup.done'
    }
    ...
    afterUpdate(() => {
        ...
        document.title = $t('trans.webTitle')
    })

    $:{ // reactive change disabled status
        optionMenuItem = [{
            label:'trans.optionMenuItem.previewImg',
    ...
    const uploadFile:(filesList:FileList) => void = filesList => {
        ...
        count = -1
        isUpload = true
        progressTxt = 'trans.uploadStatus.uploading'
    ...
    const downloadFile:(url:string) => Promise<void> = async url => {
        ...
        const resp = await fetch(`/download?f=${btoa(encodeURIComponent(url))}`)
    ...
    const previewMedia:(url:string,isImg: boolean) => void = (url,isImg) => {
        if(isImg){
            window.open(`/preview?f=${btoa(encodeURIComponent(url))}`)
            return
        }

        fetch(`/api/convert?f=${btoa(encodeURIComponent(url))}`)
            .then(res => res.json())
            .then(async res => {
                if(res.message === 'success'){
                    mediaM3U8Path = `${atob(decodeURIComponent(res.result))}`
                }
            })
            .catch(err => console.log(err))
    }
    ...
    const actionDictionary:() => void = () => {
        ...
        const replaceTextPlatform = import.meta.env.VITE_APP_PLATFORM === 'win32' ? '\\' : '/'

        currentDirTemp = currentDirTemp.replace(`${replaceTextPlatform}${currentDirTempRef}`,'')
    ...
    const settingEvent:() => Promise<void> = async () => {
        ...
        if(createUploadPath){
            const resultI = await fetch('/api/create_setting',{ 
                method:'post',
                headers:{ "Content-Type":"application/json" },
    ...
    const initFetch:() => Promise<void> = async () => {
        const resFirst = await fetch('/api/check_setting_exist')
        ...
        const res = await fetch('/api/get_upload_dirs',{ method:'post',headers:{ "Content-Type":"application/json" },body:JSON.stringify({ folderUrl:'' }) })
        const resItem = await res.json()

        const resIp = await fetch('/api/get_remote_ip')
...
<script lang="ts" context="module">
    ...
    export const load = async ({ fetch,url:{ pathname } }:LoadEvent) => {
        // const res = await fetch('/api/get_upload_dirs',{ method:'post',headers:{ "Content-Type":"application/json" },body:JSON.stringify({ folderUrl:'' }) })
        // const resItem = await res.json()
